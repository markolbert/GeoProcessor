...
namespace J4JSoftware.GeoProcessor
{
    ...
    public class KMZImporter : KMLImporter, IImporter
    {
        public KMZImporter( AppConfig config, IJ4JLogger logger )
            : base( config, logger )
        {
            Type = KMLExtensions.GetTargetType<ImporterAttribute>(GetType())!.Type;
        }

        public override async Task<List<KmlDocument>?> ImportAsync( string filePath, CancellationToken cancellationToken )
        {
